...
impl KubernetesSessionHandle {
    ...
    /// otherwise closing a correlated session (admin close, or its ticket
    /// being revoked) would only stop *new* requests from being admitted, not
    /// the potentially long-lived one already streaming.
